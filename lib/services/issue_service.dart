...
class IssueService {
  ...
  /// Returns lightweight [MapIssue] objects for rendering markers.
  Future<List<MapIssue>> getMapIssues({
    ...
    try {
      ...
      final response = await _apiClient.get(
        Uri(path: '/issues/map', queryParameters: query).toString(),
      );

      if (response.statusCode == 200) {
        final List data = jsonDecode(response.body);
        return data
            .map((item) => MapIssue.fromJson(item as Map<String, dynamic>))
            .toList();
      }
      return [];
    } catch (e) {
      debugPrint('Get map issues error: $e');
      return [];
    }
  }
