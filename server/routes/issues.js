...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    const lng = parseFloat(longitude) || 0.0;
...
// @desc    Get issues within map viewport bounds (lightweight payload for markers)
// @access  Public
router.get('/map', async (req, res) => {
  const { swLng, swLat, neLng, neLat, status, category, since } = req.query;

  const bounds = [swLng, swLat, neLng, neLat].map(Number);
  const [southwestLongitude, southwestLatitude, northeastLongitude, northeastLatitude] = bounds;

  if (
    bounds.some((value) => !Number.isFinite(value)) ||
    southwestLatitude < -90 || southwestLatitude > 90 ||
    northeastLatitude < -90 || northeastLatitude > 90 ||
    southwestLongitude < -180 || southwestLongitude > 180 ||
    northeastLongitude < -180 || northeastLongitude > 180 ||
    southwestLatitude > northeastLatitude
  ) {
    return res.status(400).json({ message: 'Viewport bounds (swLng, swLat, neLng, neLat) are required' });
  }
  ...
  try {
    ...
    if (since) {
      ...
      if (Number.isNaN(sinceDate.getTime())) {
        return res.status(400).json({ message: 'since must be a valid ISO date' });
      }
      filter.createdAt = { $gte: sinceDate };
    }

    const issues = await Issue.find(filter)
      .select('_id location category status imageUrl title address createdAt')
      .sort({ createdAt: -1 })
      .limit(200)
      .lean();
